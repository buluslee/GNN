...
def skip_gram_u(center, contexts, negs, node_list_u, lam, pa):
    """
    skip-gram
    :param center:
    :param contexts:
    ...
    for node in negs:
        I_z[node] = 0
    V = np.array(node_list_u[contexts]['embedding_vectors'])
    update = [[0] * V.size]
    for u in I_z.keys():
        if node_list_u.get(u) is  None:
            pass
        Theta = np.array(node_list_u[u]['context_vectors'])
        X = float(V.dot(Theta.T))
        sigmod = 1.0 / (1 + (math.exp(-X * 1.0)))
        update += pa * lam * (I_z[u] - sigmod) * Theta
        node_list_u[u]['context_vectors'] += pa * lam * (I_z[u] - sigmod) * V
        try:
            loss += pa * (I_z[u] * math.log(sigmod) + (1 - I_z[u]) * math.log(1 - sigmod))
        except:
            pass
...
def skip_gram_v(center, contexts, negs, node_list_v, lam, pa):
    """
    skip-gram
    :param center:
    :param contexts:
    ...
    for node in negs:
        I_z[node] = 0
    V = np.array(node_list_v[contexts]['embedding_vectors'])
    update = [[0] * V.size]
    for v in I_z.keys():
        if node_list_v.get(v) is  None:
            pass
        Theta = np.array(node_list_v[v]['context_vectors'])
        X = float(V.dot(Theta.T))
        sigmod = 1.0 / (1 + (math.exp(-X * 1.0)))
        update += pa * lam * (I_z[v] - sigmod) * Theta
        node_list_v[v]['context_vectors'] += pa * lam * (I_z[v] - sigmod) * V
        try:
            loss += pa * (I_z[v] * math.log(sigmod) + (1 - I_z[v]) * math.log(1 - sigmod))
        except:
            pass
            # print "skip_gram:",
            # print(V,Theta,sigmod,X,math.exp(-X * 1.0),round(math.exp(-X * 1.0),10))
